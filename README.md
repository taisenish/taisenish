# Taise Nish

 Informatics (B.S.) @ University of Washington — Data Science  
Software Engineering | Systems | Practical Applications

## About Me
I’m an Informatics student graduating June 2026 with a strong foundation in
data structures, algorithms, and system design. I enjoy building software that
solves real problems, from optimized graph algorithms to business-facing tools.

## Featured Projects
### Inventory Management System
- Built a C++ inventory system to track products, quantities, and updates
- Focused on correctness, efficiency, and clean system design  
 https://github.com/taisenish/inventorysystem

### MiniGit
- Implemented a simplified version-control system
- Reinforced understanding of file systems, commits, and state tracking  
 https://github.com/taisenish/minigit-123

### NYE Flights Web Project
- Web-based project focused on frontend structure and user interaction  
https://github.com/taisenish/nyeflights-371


## Skills
- **Languages:** Java, C++, JavaScript, HTML/CSS
- **Concepts:** Data Structures, Graphs, Algorithms, System Design
- **Tools:** Git, GitHub, CI/CD, Testing

## Links and Documents
- LinkedIn: https://linkedin.com/in/taise-nish-772148349
- Resume :  [TaiseKNish-RESUME(2026).pdf](https://github.com/user-attachments/files/24754070/TaiseKNish-RESUME.2026.pdf)
